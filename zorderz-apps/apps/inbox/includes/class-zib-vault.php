<?php
/**
 * ZIB_Vault — per-account encrypted token store + single-flight refresh.
 *
 * A faithful clone of ZSCH_Vault (Connected Calendars Phase 0). The two hard
 * parts, unchanged because they were already right:
 *
 *   1. ENCRYPTION AT REST. Tokens are sealed with libsodium secretbox
 *      (XSalsa20-Poly1305) when available, else OpenSSL AES-256-CTR + an
 *      HMAC-SHA256 tag (encrypt-then-MAC). The key derives from
 *      wp_salt('auth') + a domain string unique to this plugin, so a DB dump
 *      alone can never yield a usable token (INV-4). Values are prefixed
 *      ('v1s:' / 'v1o:') so both schemes decrypt side by side.
 *
 *   2. SINGLE-FLIGHT REFRESH (INV-4, the FreshBooks lesson). Microsoft ROTATES
 *      the refresh token on every use, so a lost rotation strands the grant.
 *      One MySQL GET_LOCK per account serializes the refresh; the holder
 *      re-reads and ADOPTS a sibling's fresh token (token_version bump) rather
 *      than refreshing twice; waiters poll briefly and adopt.
 *
 * DIFFERENCES FROM THE SCHEDULER: one provider (microsoft) and one scope tier
 * (Mail.Read — read only), so there is no write-scope branch on refresh.
 * `invalid_grant` (revoked / password change / 90-day idle) → status
 * 'reauth_needed', tokens kept, quiet Reconnect chip. Token VALUES never
 * appear in a log — lengths and versions only.
 *
...
class ZIB_Vault {
	...
	private static function key(): string {
		return hash( 'sha256', wp_salt( 'auth' ) . 'zib-vault-v1', true );
	}

	private static function mac_key(): string {
		return hash( 'sha256', wp_salt( 'auth' ) . 'zib-vault-mac-v1', true );
	}
	...
	/** Open a sealed value. Returns '' for empty/undecryptable input. */
	...
	/** Round-trip sanity: never store a value we cannot read back. */
	...
	/**
	 * Persist a (rotated) token pair atomically and bump the version.
	 * Empty $refresh keeps the stored refresh token.
	 */
	...
	/**
	 * Get a valid access token for an account, refreshing if needed. The ONLY
	 * correct way to obtain a token for Graph calls.
	 *
	...
	public static function get_access_token( int $account_id ) {
		...
		if ( ! $row ) {
			return new WP_Error( 'zib_vault_missing', 'Unknown mailbox account.' );
		}
	...
	private static function refresh_single_flight( $row ) {
		global $wpdb;
		$lock = 'zib_tok_' . (int) $row->id . '.' . DB_NAME;
		...
		if ( '1' !== (string) $got ) {
			...
			return new WP_Error( 'zib_vault_busy', 'Mailbox account is busy refreshing — try again.' );
		}
		...
		try {
			...
			if ( is_wp_error( $result ) ) {
				...
				return $result; // Transient (network/5xx/invalid_client): do NOT blank anything.
			}

			$ok = self::store_tokens(
				(int) $row->id,
				(string) $result['access_token'],
				(string) ( $result['refresh_token'] ?? '' ), // MS rotates.
				(int) ( $result['expires_in'] ?? 3600 )
			);
			if ( ! $ok ) {
				return new WP_Error( 'zib_vault_store', 'Could not persist refreshed token.' );
			}
