...
- `ZANA_Background`: runs that **same** turn off the browser request. Enqueue writes a
  job row, fires a non-blocking loopback (`admin-ajax`, `timeout 0.01`), the loopback
  runs `ZANA_Chat::send()` and stores the result, the client polls. Lets a slow
  (vault-augmented / thinking) turn finish without holding the request open past a
  managed host's ~60s gateway timeout (the 502). Additive: the sync path is unchanged.
- `ZANA_REST`: routes under `ZDZ_REST_NS` (`zorderz/v1/analytics/*`), access-gated.
  Sync `POST .../chat` is intact; async adds `POST .../chat/enqueue` and
  `GET .../turn/<id>`, gated by the same `can_access` callback.
...
- **token streaming** (the async **job queue + polling** now ships, see
  `ZANA_Background`; the client uses it and falls back to the sync route when it is
