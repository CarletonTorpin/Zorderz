...
- `zorderz-theme-1.3.3.zip`: the theme (platform kernel + Core services)
- `zorderz-apps-1.3.3.zip`: the apps bundle (18 apps)

**Verify** each is an intact zip with the expected top-level folder:

```bash
unzip -Z1 zorderz-theme-1.3.3.zip | head -1     # -> zorderz/
unzip -Z1 zorderz-theme-1.3.3.zip | grep -m1 'zorderz/style.css'
unzip -Z1 zorderz-apps-1.3.3.zip  | head -1     # -> zorderz-apps/
unzip -Z1 zorderz-apps-1.3.3.zip  | grep -m1 'zorderz-apps/zorderz-apps.php'
```
...
```bash
wp theme install /path/to/zorderz-theme-1.3.3.zip --activate
```
...
wp theme get zorderz --field=version              # -> 1.3.3
```
...
```bash
wp plugin install /path/to/zorderz-apps-1.3.3.zip --activate
```
...
wp plugin get zorderz-apps --field=version        # -> 1.3.3
```
...
- [ ] `wp theme get zorderz --field=version` → `1.3.3`, and the theme is active.
- [ ] `wp plugin get zorderz-apps --field=version` → `1.3.3`, and the plugin is active.
...
```bash
wp theme install /path/to/zorderz-theme-1.3.3.zip --force
wp plugin install /path/to/zorderz-apps-1.3.3.zip --force
wp rewrite flush --hard
```
