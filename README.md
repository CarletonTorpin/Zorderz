...
2. **Upload the theme.** Download the two zips from the [latest release](https://github.com/CarletonTorpin/Zorderz/releases/latest); they are named `zorderz-theme-<version>.zip` and `zorderz-apps-<version>.zip`, one version for both. Then Appearance → Themes → Add New → Upload Theme → the theme zip → Install → **Activate**. That is the only upload you need: the theme installs and activates the apps for you. (If your host blocks the theme from writing to the plugins folder, a notice tells you to upload the apps zip under Plugins → Add New; everything else is the same.)
...
**Appearance → Themes → Add New → Upload Theme**, choose `zorderz-theme-<version>.zip`, install, and **Activate**.
...
On most hosts there is nothing to do here: activating the theme in step 2 already installed and activated the apps. Do this by hand only if a notice told you the theme could not write to the plugins folder, in which case: **Plugins → Add New → Upload Plugin**, choose `zorderz-apps-<version>.zip`, install, and **Activate**.
